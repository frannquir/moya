...
-- camino documentado de una fila guardada a un cronograma -- tiraba RangeError
-- sin catch. En un Server Component eso es un 500, no un mensaje. (planDeFila()
-- además se endureció para no lanzar: esto es la primera valla, no la única.)
--
...
--
-- POR QUÉ VA EN SU PROPIO ARCHIVO, Y CON ESTE NÚMERO
--   * No dentro de 20260923120000: esa migración ya está aplicada y registrada
--     en la base, así que `supabase db push` la saltea y editarla no haría nada.
--   * No con 20260924120000: ese número ya lo usa
--     20260924120000_escritos_cedula_domicilio_claves.sql, que entró a `main`
--     con el paquete C. Supabase indexa las migraciones por ese prefijo y no por
--     el nombre completo, así que dos archivos con el mismo número rompen el push.
--
